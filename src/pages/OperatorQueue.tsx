...
export function OperatorQueue() {
  ...
  // Always fetch all — filter client-side for counts + period
  const { requests: allRequests, isLoading, error, cancelRequest } = useMyStockRequests('all')
  ...
  // Cancel button only shown on the "All" tab (pending items surface there)
  const showCancelColumn = activeTab === 'all'
